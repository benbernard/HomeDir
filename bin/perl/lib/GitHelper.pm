...

{
    ...
    sub ensure_hub_config {
        if (not $has_run) {
            set_config('hub.host', 'github.groupondev.com');
...

{
    ...
    sub needs_tunnel {
        if(run_command('curl', 'https://github.groupondev.com') =~ m/gdwall/) {
            return 1;
        }
        return 0;
    }
    ...
    sub open_tunnel {
        ...
        my $user = (getpwuid($<))[0];
        system("sudo ssh -f -N -L443:github.groupondev.com:443 -i ~/.ssh/id_rsa $user\@b.snc1.groupon.com");
        run_command("sudo cp /etc/hosts /etc/hosts.orig");
        run_command("sudo sh -c 'echo 127.0.0.1	github.groupondev.com >> /etc/hosts'");
    }

    sub close_tunnel {
        my $force = shift;
        return if (!$force && !$tunnel_is_up);
        run_command("ps -ef | grep 'ssh.*github.groupondev.com:443' | grep -v grep | awk '{print \$2}' | sudo xargs kill -9");
        if ( -e '/etc/hosts.orig' ) {
