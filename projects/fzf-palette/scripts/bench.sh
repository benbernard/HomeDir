...
MOVEMENT_JSON="$("$CLI" bench movement --runs "$MOVEMENT_RUNS" --warmup "$MOVEMENT_WARMUP" --json)"
echo "$MOVEMENT_JSON"
...
python3 - "$PANEL_JSON" "$HOTKEY_JSON" "$CARBON_HOTKEY_JSON" "$KEYSTROKE_JSON" "$LARGE_KEYSTROKE_JSON" "$MOVEMENT_JSON" "$MAIN_THREAD_JSON" "$SOURCE_JSON" "$PREVIEW_JSON" "$RESULT_JSON" "$LIFECYCLE_JSON" "$ROUNDTRIP_JSON" <<'PY'
import json
import sys
