...
sub do_auto_head
{
    ...
    if ($res->code ne RC_OK) {
        $window->printformat(MSGLEVEL_CRAP, 'url_auto_head', $res->status_line(), $number);
    } else {
        ...
        $window->printformat(MSGLEVEL_CRAP, 'url_auto_head', $t, $number);
    }
}
...
#        }
        ...
        if ( Irssi::settings_get_bool("url_log_auto_head") ) {
            do_auto_head($2, $window, (scalar @urls) - 1);
        }
...
sub sig_url
{
    ...
    if (@args <= 0) {
        ...
        print_msg "    NINJA <SEARCH TERM> Search a term with ninja serach";
...
Irssi::theme_register(['url_head', '[%gHTTP Head%n %g$0%n]$1-',
                       'url_auto_head', 'Item: $1 - [%gHEAD%n] $0-',
