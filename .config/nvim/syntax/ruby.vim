...
syn cluster rubyNumber contains=rubyASCIICode,rubyInteger,rubyFloat
...
" Any identifier
syn cluster rubyIdentifier contains=rubyConstant,rubyClassVariable,rubyInstanceVariable,rubyGlobalVariable,rubySymbol,rubyIterator,rubyPredefinedVariable,rubyPredefinedConstant
...
" We need to balance {<[( characters within % delimited stuff.
" UnInterp for stuff that's not interpolated (contains no variable substitution)
syn region rubyBracesUnInterp start="{" end="}" contained transparent contains=rubyBracesUnInterp
syn region rubyAnglesUnInterp start="<" end=">" contained transparent contains=rubyAnglesUnInterp
syn region rubyBracketsUnInterp start="\[" end="]" contained transparent contains=rubyBracketsUnInterp
syn region rubyParensUnInterp start="(" end=")" contained transparent contains=rubyParensUnInterp

syn region rubyBracesInterp start="{" end="}" contained transparent contains=@rubyStringSpecial,rubyBracesInterp
syn region rubyAnglesInterp start="<" end=">" contained transparent contains=@rubyStringSpecial,rubyAnglesInterp
syn region rubyBracketsInterp start="\[" end="]" contained transparent contains=@rubyStringSpecial,rubyBracketsInterp
syn region rubyParensInterp start="(" end=")" contained transparent contains=@rubyStringSpecial,rubyParensInterp
...
syn region rubyString matchgroup=rubyStringDelimiter start="%r{"				end="}[iomx]*"	 skip="\\\\\|\\}"   contains=@rubyStringSpecial,rubyBracesInterp fold
syn region rubyString matchgroup=rubyStringDelimiter start="%r<"				end=">[iomx]*"	 skip="\\\\\|\\>"   contains=@rubyStringSpecial,rubyAnglesInterp fold
syn region rubyString matchgroup=rubyStringDelimiter start="%r\["				end="\][iomx]*"	 skip="\\\\\|\\\]"  contains=@rubyStringSpecial,rubyBracketsInterp fold
syn region rubyString matchgroup=rubyStringDelimiter start="%r("				end=")[iomx]*"	 skip="\\\\\|\\)"   contains=@rubyStringSpecial,rubyParensInterp fold

" Generalized Single Quoted String, Symbol and Array of Strings
syn region rubyString matchgroup=rubyStringDelimiter start="%[qsw]\z([~`!@#$%^&*_\-+=|\:;"',.?/]\)" end="\z1" skip="\\\\\|\\\z1" fold
syn region rubyString matchgroup=rubyStringDelimiter start="%[qsw]{"				    end="}"   skip="\\\\\|\\}"	 fold contains=rubyBracesUnInterp
syn region rubyString matchgroup=rubyStringDelimiter start="%[qsw]<"				    end=">"   skip="\\\\\|\\>"	 fold contains=rubyAnglesUnInterp
syn region rubyString matchgroup=rubyStringDelimiter start="%[qsw]\["				    end="\]"  skip="\\\\\|\\\]"	 fold contains=rubyBracketsUnInterp
syn region rubyString matchgroup=rubyStringDelimiter start="%[qsw]("				    end=")"   skip="\\\\\|\\)"	 fold contains=rubyParensUnInterp
...
syn region rubyString matchgroup=rubyStringDelimiter start="%[QWx]\={"				    end="}"   skip="\\\\\|\\}"	 contains=@rubyStringSpecial,rubyBracesInterp fold
syn region rubyString matchgroup=rubyStringDelimiter start="%[QWx]\=<"				    end=">"   skip="\\\\\|\\>"	 contains=@rubyStringSpecial,rubyAnglesInterp fold
syn region rubyString matchgroup=rubyStringDelimiter start="%[QWx]\=\["				    end="\]"  skip="\\\\\|\\\]"	 contains=@rubyStringSpecial,rubyBracketsInterp fold
syn region rubyString matchgroup=rubyStringDelimiter start="%[QWx]\=("				    end=")"   skip="\\\\\|\\)"	 contains=@rubyStringSpecial,rubyParensInterp fold
...
if !exists("ruby_no_expensive")
  ...
  syn region rubyBlock start="\<def\>"    matchgroup=rubyDefine end="\<end\>" contains=@rubyTopLevel nextgroup=rubyFunction fold
  syn region rubyBlock start="\<class\>"  matchgroup=rubyDefine end="\<end\>" contains=@rubyTopLevel nextgroup=rubyClass    fold
  syn region rubyBlock start="\<module\>" matchgroup=rubyDefine end="\<end\>" contains=@rubyTopLevel nextgroup=rubyModule   fold

  " modifiers
  syn match  rubyControl "\<\%(if\|unless\|while\|until\)\>" display

  " *do* requiring *end*
  syn region rubyDoBlock matchgroup=rubyControl start="\%(\<\%(for\|until\|while\)\s.*\s\)\@<!do\>" end="\<end\>" contains=@rubyTopLevel fold

  " *{* requiring *}*
  syn region rubyCurlyBlock start="{" end="}" contains=@rubyTopLevel fold

  " statements without *do*
  syn region rubyNoDoBlock matchgroup=rubyControl start="\<\%(case\|begin\)\>" start="^\s*\%(if\|unless\)\>" start="[;=(]\s*\%(if\|unless\)\>"hs=s+1 end="\<end\>" contains=@rubyTopLevel fold

  " statement with optional *do*
  syn region rubyOptDoBlock matchgroup=rubyControl start="\<for\>" start="^\s*\%(while\|until\)\>" start=";\s*\%(while\|until\)\>"hs=s+1 end="\<end\>" contains=@rubyTopLevel fold
...
syn cluster rubyBlockConstruct contains=rubyFunction,rubyClass,rubyDefine,rubyModule,rubyBlock,rubyControl,rubyDoBlock,rubyCurlyBlock,rubyNoDoBlock,rubyOptDoBlock,rubyControl
...
" Keywords & keyword like methods
syn cluster rubyKeywordSpecial contains=rubyControl,rubyOperator,rubyKeyword,rubyBoolean,rubyPseudoVariable,rubyBeginEnd,rubyAccess,rubyAttribute,rubyEval,rubyException,rubyInclude
...
" Cluster for inclusion in blocks &c
syn cluster rubyTopLevel contains=@rubyNumber,@rubyIdentifier,rubyString,@rubyBlockConstruct,@rubyKeywordSpecial,rubyComment,rubyDocumentation,rubyKeywordAsMethod,rubyData
